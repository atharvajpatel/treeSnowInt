...
export class TreeVisualizer {
    ...
    async getFileDiff(file, commitData) {
        try {
          // Make sure commitData.owner, commitData.repo, and commitData.sha are set
          const { owner, repo, sha } = commitData;
          if (!owner || !repo || !sha) {
            return 'Missing owner/repo/sha in commit data.';
          }
      
          // Call the GitHub commits API: /repos/{owner}/{repo}/commits/{sha}
          const url = `https://api.github.com/repos/${owner}/${repo}/commits/${sha}`;
          const headers = {
            Accept: 'application/vnd.github.v3+json',
          };
          // If you have a GitHub token to avoid rate-limits / private repos:
          ...
          // The `files` array in the JSON has info for each changed file, including `filename` and `patch`
          const matchedFile = commitJson.files.find(f => f.filename === file);
          ...
          // Return the patch as colorized HTML lines
          return this.formatDiff(matchedFile.patch);
      
        } catch (error) {
          console.error('Error fetching diff:', error);
          return 'Unable to load diff content from GitHub API.';
        }
      }
      ...
      formatDiff(diffContent) {
        ...
        // Convert each line to a <div> with color-coded background
        return diffContent.split('\n').map(line => {
          const safeLine = this.escapeHtml(line);  // or your existing escapeHtml
          if (line.startsWith('+')) {
            // Addition
            return `<div class="diff-line diff-line-add" style="background:#e6ffed;">${safeLine}</div>`;
          } else if (line.startsWith('-')) {
            // Deletion
            return `<div class="diff-line diff-line-del" style="background:#ffeef0;">${safeLine}</div>`;
          } else {
            // Context or unchanged line
            return `<div class="diff-line">${safeLine}</div>`;
          }
        }).join('\n');
      }
